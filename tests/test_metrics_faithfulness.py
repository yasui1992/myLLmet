...
def mock_clients():
    ...
    mock_claim_extractor.invoke.return_value = {"claims": ["c1", "c2"]}
    mock_faithfulness_judge.invoke.return_value = {
        "verdicts": [
...
def test_mismatched_claims_and_verdicts_failed(mock_clients):
    ce_client, fj_client = mock_clients
    ce_client.invoke.return_value = {"claims": ["c1", "c2", "c3"]}
    fj_client.invoke.return_value = {
        "verdicts": [
            {"claim": "c1", "verdict": 1, "reason": "r1"},
            {"claim": "c2", "verdict": 0, "reason": "r2"}
        ]
